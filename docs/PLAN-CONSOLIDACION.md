...
| 4 | TOC con anclas: reimplementar en `cmark_gfm_rendering.m` (arregla el TOC roto de la experimental) | Alto | ✅ `v0.9-cons.8` — CI verde + **verificado en runtime** (Copy HTML sobre `prueba-toc.md`: cada `href` casa con su `id`). Incluye: cherry-pick de Reza (anclas slug), `toc_N` para el macro `[TOC]`, fix de mojibake UTF-8 en el texto del TOC y **plegado de acentos** (decisión de UX, diverge de GitHub: `Instalación`→`#instalacion`, ñ→n) |
