# Índice de documentación — MacDown (fork propio)

- [CLAUDE.md](CLAUDE.md) — Aproximación, objetivos, modelo de git, build y tracking de forks.
- [docs/FORKS.md](docs/FORKS.md) — Estado del ecosistema de forks (generado por `claude_tools/track_forks.sh`).
- [docs/DIARY.md](docs/DIARY.md) — Diario de desarrollo.
- [README.md](README.md) — README original de MacDown.

## Utilidades

- [`claude_tools/track_forks.sh`](claude_tools/track_forks.sh) — Regenera el informe de forks y el snapshot.
- [`claude_tools/seeds.txt`](claude_tools/seeds.txt) — Repos semilla a rastrear.
